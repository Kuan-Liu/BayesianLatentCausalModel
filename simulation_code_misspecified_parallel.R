...
if( !file.exists("model_norm_indicators_sen.txt") ){
  cat( " model {
     #N = nobs
     for (i in 1:N) {
    ...
    for (h in 1:(nindic)) {
     ...
     t6 ~ dnorm(0,0.1)
       ...
       file = "model_norm_indicators_sen.txt")
}


mysim <- function(outfile, from=1, to=100, nobs=500, nindic=10, qindic=1, samplesize=10000) { 
  ...
  results<-foreach(i=from:to, .combine='rbind',.inorder=F, .noexport=NULL, .verbose=T) %dopar% {
    ...
    .libPaths(c(.libPaths(),"/home/k/kuan/kuan/R","/home/k/kuan/kuan/R/x86_64-pc-linux-gnu-library/4.1"))
    ...
    for (p in 1:nindic) {
      indic1[, p] <- rbinom(nobs, size = 1, prob = expit(0.5-0.1*c_a+0.2*c_s))
    }
    
    pz1 <- expit(-1+0.1*rowSums(indic1))
    z1 <- rbinom(nobs,1,prob=pz1)
    
    #second visit;
    indic2<-matrix(NA, nobs, nindic)
    for (p in 1:nindic) {
      indic2[, p] <- rbinom(nobs, size = 1, prob = expit(0.5-0.1*c_a+0.2*c_s+indic1[,p]-1*z1))
    }
    
    pz2 <- expit(-1+0.1*rowSums(indic2)-1*z1)
    z2 <- rbinom(nobs,1,prob=pz2)
    
    my<-0.1+0.5*z1+1*z2-0.1*rowSums(indic1)-0.2*rowSums(indic2) #moderate confounding effect
    y<-rnorm(nobs, mean=my, sd = 1)
    
    dat<-data.frame(y, c_a,c_s,indic1, indic2, z1,z2)
    ...
    mytrue<-function(myz1,myz2){
      ...
      indic1_sim<-matrix(NA, samplesize, nindic)
      for (p in 1:nindic) {
        indic1_sim[, p] <- rbinom(samplesize, size = 1, prob = expit(0.5-0.1*c_a_sim+0.2*c_s_sim))
      }
      
      #second visit;
      indic2_sim<-matrix(NA, samplesize, nindic)
      for (p in 1:nindic) {
        indic2_sim[, p] <- rbinom(samplesize, size = 1, prob = expit(0.5-0.1*c_a_sim+0.2*c_s_sim+indic1_sim[,p]-1*myz1))
      }
      
      my<-0.1+0.5*myz1+1*myz2-0.1*rowSums(indic1_sim)-0.2*rowSums(indic2_sim)+rnorm(samplesize,0,1) #moderate confounding effect
    ...
    #adjust regressions, adjusting covariates;
    ...
    #                              basecovs = c("L1","L2"), nsimul = nrep, seed = 1234, nsamples = nobs, parallel = TRUE, ncores = 5)
    #   
    ...
    #                              int_descript = int_descript, ref_int = 1,
    #                              histories = histories, histvars = histvars,
    #                              basecovs = c("L1","L2"), nsimul = nrep, seed = 1234, nsamples = nobs, parallel = TRUE, ncores = 8)
    #   
    #   results.it[1,22]<-gform_cont_eof$result$`Mean difference`[3]
    #   results.it[1,23]<-gform_cont_eof$result$`g-form mean`[2]
    #   results.it[1,24]<-gform_cont_eof$result$`g-form mean`[3]
    ...
    if (nindic==5){
      ...
      tmle_model <- ltmle (datt ,Anodes = c ("A1" , "A2") , 
                           ...
                           gform=c("A1 ~ W1 + W2 + L1.1 + L2.1 + L3.1 + L4.1 + L5.1",
                                   "A2 ~ A1 + W1 + W2 + L1.2 + L2.2 + L3.2 + L4.2 + L5.2"),
    ...
    jags.data<-list(c_a= c_a, c_s=c_s,
                    indic1=indic1, z1= z1,
                    indic2=indic2, z2= z2, y= y, 
                    N = nobs, nindic=nindic)
    ...
    jagsfit<- jags(data = jags.data, jags.inits, jags.params, n.iter = 50000,
                   model.file = "model_norm_indicators_sen.txt", n.chains = 1, n.burnin = 40000, n.thin = 5)
...
mysim(paste0("n500","_nindic10","_missspecified"), from = 1, to = 1000, nobs = 500, nindic = 10, samplesize=10000)
mysim(paste0("n500","_nindic5","_missspecified"), from = 1, to = 1000, nobs = 500, nindic = 5, samplesize=10000)
